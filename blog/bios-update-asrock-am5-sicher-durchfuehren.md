...
description: "Ein spontan neu startender PC ohne Bluescreen führt fast immer zur Empfehlung: BIOS aktualisieren. Diese Anleitung zeigt den kompletten Ablauf am Beispiel eines ASRock-AM5-Boards: Version ermitteln, Download per Hash verifizieren, BitLocker korrekt pausieren, ins UEFI booten (auch wenn F2 ins Leere läuft) und die Einstellungen nach dem Update sinnvoll setzen."
date: "2026-08-26"
...
Ein Desktop-PC startet spontan neu, ohne Bluescreen, ohne Absturzabbild, im Ereignisprotokoll steht nur Kernel-Power 41 mit `BugcheckCode=0`. Wer dieses Muster recherchiert, landet bei AM5-Systemen fast immer bei derselben Empfehlung: zuerst das BIOS aktualisieren, denn die AGESA-Firmware von AMD bekommt laufend Korrekturen für Speicher- und Spannungsprobleme, die genau solche Abstürze verursachen. Diese Anleitung dokumentiert den kompletten Ablauf an einem konkreten Beispiel: ein ASRock A620I Lightning WiFi wird von BIOS 3.30 auf 4.43 gebracht. Die Schritte übertragen sich auf jedes moderne Mainboard, die Fallstricke (BitLocker, Fast Boot, Einstellungs-Reset) sind herstellerunabhängig.

## Warum das BIOS bei Stabilitätsproblemen zuerst dran ist

Ein Neustart ohne Bugcheck bedeutet: Windows hat den Absturz weder verursacht noch protokolliert. Das System ist auf Hardware- oder Firmware-Ebene ausgefallen, typischerweise durch instabile Spannungen, Speichertraining oder Firmware-Fehler. Genau diese Ebene pflegt AMD über AGESA-Releases, die die Board-Hersteller in ihre BIOS-Versionen einbetten. Im Beispiel liegen zwischen der installierten Version 3.30 und der aktuellen 4.43 mehrere Versionen, deren Changelogs auffällig gut zum Fehlerbild passen: "Improve memory compatibility and system stability" (3.40), "Optimized Memory Compatibility" (4.10) und ein überarbeitetes EXPO-Handling (4.41). Ein BIOS-Update ist damit die Massnahme mit dem besten Verhältnis von Aufwand zu Erfolgsaussicht.
...
Die Lösung ist eingebaut und dauert eine Sekunde. In einer PowerShell mit Administratorrechten:
...
Die erste Zeile muss die neue Version zeigen, die zweite den erwarteten Speichertakt, und BitLocker muss wieder "Schutz aktiviert" melden. Damit ist das Update abgeschlossen und dokumentiert; ob es die Neustarts tatsächlich behoben hat, zeigt erst die Beobachtung über die folgenden Wochen, am einfachsten mit einem Blick auf neue Kernel-Power-41-Einträge im System-Ereignisprotokoll.
