---
title: "Midea PortaSplit in Home Assistant integrieren: lokale Steuerung, Einrichtung und Security-Risiken"
navTitle: "PortaSplit in HA"
description: "Die Midea PortaSplit lässt sich per Community-Integration lokal in Home Assistant steuern. Welche Integration passt, weshalb bei der Einrichtung trotzdem kurz die Midea-Cloud nötig ist, was Token und Key wirklich bedeuten und wie sich das Gerät sicher im Heimnetz betreiben lässt."
date: "2026-07-24"
...
<aside class="article-update">
  <p class="article-update__label">Nachtrag (Juli 2026): Token-Schnittstellen werden abgeschaltet</p>
  <p>Das Projekt Midea AC LAN warnt: Midea schaltet die Cloud-Schnittstellen ab, über die Home Assistant bei der Einrichtung den gerätespezifischen Token und Key bezieht. Für PortaSplit-Besitzer heisst das:</p>
  <ol>
    ...
    <li><strong>Token, Key und die JSON-Konfiguration verschlüsselt sichern.</strong> Nach der Abschaltung sind diese Werte voraussichtlich nicht erneut beschaffbar; das Backup ist dann der einzige Weg zu einer Neueinrichtung.</li>
  ...
  <p>Bereits eingerichtete Geräte steuern Home Assistant lokal weiter; betroffen ist nach heutigem Stand das Hinzufügen, nicht der Betrieb. Hintergründe im Abschnitt <a href="#die-warnung-von-midea-ac-lan">Die Warnung von Midea AC LAN</a>, die Backup-Schritte unter <a href="#backup-der-konfiguration">Backup der Konfiguration</a>.</p>
</aside>
...
Die Midea PortaSplit gehört zu den interessantesten mobilen Klimageräten für Mietwohnungen: Der Kompressor sitzt draussen, das Innengerät arbeitet vergleichsweise leise im Raum, und für die Montage braucht es keinen Eingriff in die Bausubstanz. Weniger bekannt ist, dass sich die PortaSplit auch in Home Assistant einbinden lässt und sich damit abhängig von Raumtemperatur, Strompreis, Fensterstatus oder Anwesenheit steuern lässt.

Die Integration funktioniert nach der Einrichtung weitgehend lokal im eigenen Netzwerk. Ganz ohne Cloud und ganz ohne Sicherheitsfragen kommt sie jedoch nicht aus. Besonders relevant ist eine aktuelle Warnung des Projekts Midea AC LAN: Midea schaltet schrittweise jene Cloud-Schnittstellen ab, über die Home Assistant die für die lokale Kommunikation benötigten Zugangsdaten abruft. Gleichzeitig bezeichnet der Entwickler die bisherige Token-Architektur als Sicherheitsproblem.
...
Beide kommunizieren nach erfolgreicher Einrichtung direkt über das lokale Netzwerk mit dem Klimagerät. Bei neueren Midea-Geräten wird die Cloud allerdings einmalig benötigt, um einen gerätespezifischen Token und Key zu beschaffen.

Für die PortaSplit ist `Midea Smart AC` derzeit die naheliegendere Wahl. Diese Integration weist die PortaSplit ausdrücklich als unterstütztes Gerät aus und beherrscht inzwischen auch den 2026 eingeführten „Out Silent Mode". Alternativ kommt `Midea AC LAN` infrage, das neben Klimaanlagen zahlreiche weitere Midea-Gerätetypen abdeckt.
...
## Weshalb lokale Steuerung trotzdem eine Cloud benötigt
...
Bei neueren Geräten mit dem sogenannten V3-Protokoll benötigt Home Assistant jedoch zwei gerätespezifische Werte: einen Token und einen Key. Beide dienen der Authentifizierung und Verschlüsselung der lokalen Kommunikation. Die Integration kennt sie nicht automatisch, sie holt sie während der erstmaligen Einrichtung über eine Midea-Cloud-Schnittstelle ab. Die Entwickler von `Midea Smart AC` beschreiben das ausdrücklich: Für V3-Geräte wird die Midea-Cloud während der Erkennung verwendet, um Token und Key zu erhalten. Danach werden diese lokal gespeichert, und für die weitere Steuerung ist keine Cloud-Verbindung erforderlich.
...
„Lokal steuerbar" bedeutet daher nicht automatisch „niemals mit einer Cloud verbunden".
...
Der grösste Nachteil ist derzeit die Warnung des Entwicklers: Die für das Hinzufügen neuer Geräte verwendeten Cloud-Token-APIs werden schrittweise abgeschaltet. Das spätere Hinzufügen neuer Geräte kann dadurch unmöglich werden.
...
Laut der Warnung von `Midea AC LAN` basiert die ältere LAN-Architektur auf einer problematischen Annahme: Die Entwickler gingen ursprünglich davon aus, dass die Client-Kommunikation bereits ausreichend verschlüsselt sei. Deshalb wurden die von der Cloud ausgegebenen Tokens nicht mit einer Ablaufzeit versehen.

```text
Token einmal ausgegeben → möglicherweise unbegrenzt gültig
```
...
Token und Key authentifizieren die lokale Kommunikation mit dem Gerät. Geraten beide Werte in falsche Hände, könnte ein Angreifer abhängig vom Protokoll und von seiner Netzwerkposition das Gerät erkennen, sich gegenüber dem Gerät authentifizieren, Statusinformationen auslesen, Einstellungen verändern, die Klimaanlage ein- oder ausschalten, Betriebsmodi wechseln und die Solltemperatur verändern.

Dazu muss der Angreifer in der Regel trotzdem eine Netzwerkverbindung zum Gerät herstellen können. Der Besitz von Token und Key allein ermöglicht keinen Angriff aus dem gesamten Internet. Das Risiko steigt jedoch erheblich, wenn die PortaSplit direkt aus dem Internet erreichbar gemacht wurde, Portweiterleitungen bestehen, Home Assistant kompromittiert ist, ein Angreifer Zugriff auf das lokale WLAN hat, das IoT-Netz unzureichend segmentiert ist, Backups unverschlüsselt öffentlich liegen oder Geheimnisse in ein GitHub-Repository hochgeladen werden. Token und Key sind daher wie ein Passwort zu behandeln.

### Die JSON-Datei ist kein harmloses Backup

`Midea AC LAN` legt nach erfolgreicher Einrichtung für V3-Geräte eine JSON-Konfigurationsdatei ab. Der dokumentierte Pfad lautet:
...
Bei Verwendung von `Midea AC LAN` sollte nach erfolgreicher Einrichtung die JSON-Datei gesichert werden. Über das Home-Assistant-Terminal:
...
Vollständig cloudfrei ist der Weg jedoch nicht: Neuere Geräte benötigen bei der erstmaligen Einrichtung Token und Key, die derzeit über Midea-Cloud-Schnittstellen beschafft werden. Genau diese Schnittstellen werden nach Angaben des Projekts `Midea AC LAN` schrittweise abgeschaltet. Die langfristige Zukunft der inoffiziellen lokalen API ist damit nicht garantiert. Wer die PortaSplit bereits besitzt, sollte sie zeitnah einrichten und die Konfiguration sichern. Wer sie neu kauft, sollte wissen, dass die Home-Assistant-Unterstützung auf Community-Arbeit und einem durch Reverse Engineering nachvollzogenen proprietären Protokoll basiert.
