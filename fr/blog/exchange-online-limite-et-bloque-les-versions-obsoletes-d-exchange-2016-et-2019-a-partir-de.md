---
title: "Exchange Online limite et bloque les serveurs Exchange 2016 et 2019 obsolètes à partir de septembre 2026 : fonctionnement du transport enforcement"
...
translatedAt: 2026-09-08T08:12:33.215Z
...
| `Get-OnPremServerReportInfo` | Liste les serveurs On-Premises détectés par Exchange Online avec leur build, leur état d’enforcement et leur niveau. |
...
| `-NumberOfDays 30` | Durée de la pause en jours. Le quota est de 90 jours par an, et la valeur indiquée y est déduite. |
...
`Get-ExchangeServer` n’affiche dans `AdminDisplayVersion` que le CU, et non la SU. La version de fichier de `ExSetup.exe` est fiable, tout comme le [Exchange Health Checker](https://aka.ms/ExchangeHealthChecker), qui signale également les étapes manuelles manquantes. Pour une vue d’ensemble rapide de tous les serveurs :
...
| Élément | Effet |
|---|---|
| `Get-ExchangeServer` | Liste tous les serveurs Exchange de l’organisation. |
| `\\<Server>\C$\...\ExSetup.exe` | Chemin de partage administratif vers le fichier Setup ; à adapter en cas de chemin d’installation différent. |
| `VersionInfo.ProductVersion` | Version du fichier correspondant au build de la SU installée (par ex. `15.1.2507.61`). |
...
1. **Inventoriez les niveaux** comme décrit ci-dessus, y compris les Edge Transport Server et les serveurs de gestion.
...
1.  [Exchange 2016/2019: Throttling and Blocking up to the Final Public Update Baseline – Microsoft Community Hub](https://techcommunity.microsoft.com/blog/exchange/exchange-20162019-throttling-and-blocking-up-to-the-final-public-update-baseline/4552717): L’annonce du 2 septembre 2026 concernant le nouveau seuil minimal (SU d’octobre 2025), la date de début durant la deuxième semaine de septembre et l’indication d’un prochain relèvement au-delà de la dernière mise à jour publique.
...
3.  [How to pause throttling and blocking of out-of-date on-premises Exchange Servers – Microsoft Community Hub](https://techcommunity.microsoft.com/blog/exchange/how-to-pause-throttling-and-blocking-of-out-of-date-on-premises-exchange-servers/4007169): Les cmdlets `Get-OnPremServerReportInfo`, `Get-TenantExemptionInfo` et `New-TenantExemptionInfo`, ainsi que le rapport dans l’Exchange Admin Center et le quota annuel de 90 jours.

4.  [Exchange Server build numbers and release dates – Microsoft Learn](https://learn.microsoft.com/en-us/exchange/new-features/build-numbers-and-release-dates): Numéros de build des SU d’octobre 2025 et des mises à jour ESU ultérieures jusqu’en août 2026 ; indique également que les SU à partir de décembre 2025 sont réservées aux clients ESU.
...
7.  [Support for Exchange Server 2016 and Exchange Server 2019 ends today – Microsoft Community Hub](https://techcommunity.microsoft.com/blog/exchange/support-for-exchange-server-2016-and-exchange-server-2019-ends-today/4461192): La fin du support le 14 octobre 2025.
...
9.  [Announcing Period 2 Exchange 2016/2019 Extended Security Update (ESU) program – Microsoft Community Hub](https://techcommunity.microsoft.com/blog/exchange/announcing-period-2-exchange-20162019-extended-security-update-esu-program/4511603): Période de mai à octobre 2026 et indication qu’aucune prolongation supplémentaire ne suivra.

10. [Exchange Online transport enforcement system explained – CodeTwo Admin's Blog](https://www.codetwo.com/admins-blog/persistently-vulnerable-exchange-server/): Répartition tabulaire des huit niveaux d’enforcement et des dates de déploiement pour chaque version d’Exchange ; source tierce.

11. [Exchange Server Health Checker – Microsoft CSS-Exchange](https://aka.ms/ExchangeHealthChecker): Inventaire des niveaux CU/SU et des étapes manuelles encore ouvertes.
