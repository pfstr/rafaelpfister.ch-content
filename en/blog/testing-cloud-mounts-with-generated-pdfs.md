...
**Eviction has to hit every path to the same file**: the file as the mount presents it *and* the VFS copy under `--cache-dir` (there in the `vfs/` subdirectory, not `vfsMeta/`, which holds just metadata).

**And it can fail silently.** In my control run, `fincore` showed the VFS copy fully resident in RAM after `sync` and `POSIX_FADV_DONTNEED`: 600K resident before, 600K after. Rclone keeps the cache file open, and the kernel did not release the pages. Without the `fincore` proof I would have published a "from disk" number that in truth came from RAM. To really isolate the disk layer, stop the Rclone process for the measurement or use `echo 3 > /proc/sys/vm/drop_caches` as root.
...
The same applies to the generated test files, by the way: right after generation they sit fully in the page cache. For the upload measurement that is irrelevant, since the bottleneck is the network. But anyone who wants to derive local read numbers from them has to evict first.
...
If you grep logs for error codes, choose the pattern precisely. My search for `422` returned 39 supposed hits. In fact it had matched harmless size figures such as `422504 bytes` in cleanup messages. A word-boundary pattern (`grep -E '\b422\b'`) or searching for the complete error line spares the phantom hunt.
